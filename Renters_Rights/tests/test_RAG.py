...
def test_load_source_content(mock_soup_strainer, mock_pdf_loader, mock_web_loader):
    ...
    mock_web_instance.load.return_value = [
        Document(page_content="dummy web content", metadata={"source": "web"})
    ]
    mock_web_loader.return_value = mock_web_instance

    mock_pdf_instance = MagicMock()
    mock_pdf_instance.load.return_value = [
        Document(page_content="dummy pdf content", metadata={"source": "pdf"})
    ]
...
def test_calculate_effective_date():
    ...
    result, data = calculate_effective_date.invoke(
        {"notice_date": "2026-10-08", "notice_period_days": "30"}
    )
    assert result == "2026-11-07"
    assert data["date"] == "2026-11-07"
    assert data["days"] == 30

    invalid_result = calculate_effective_date.invoke(
        {"notice_date": "invalid-date", "notice_period_days": "30"}
    )
